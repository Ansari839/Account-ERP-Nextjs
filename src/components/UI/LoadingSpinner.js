    ...
    <div className={cn("flex flex-col items-center justify-center min-h-[200px]", className)}>
        ...
        <div className="relative flex items-center justify-center mb-6">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-20">
              ...
              <div className="flex items-end h-8 space-x-1">
                {[20, 40, 60, 30, 50, 45].map((height, index) => (
                  <div
                    key={index}
                    className="bg-blue-400 rounded-t-sm animate-pulse"
                    style={{
                      width: '6px',
                      height: `${height}%`,
          ...
          <div className="relative z-10 flex items-center justify-center">
            <div className={`animate-spin-slow ${spinnerSize}`}>
              <DollarSign className="w-full h-full text-blue-600" />
            </div>
          </div>

          {/* Additional rotating elements for visual interest */}
          <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-blue-100 animate-spin-slow"
               style={{ animationDuration: '3s' }}></div>
          <div className="absolute inset-0 rounded-full border-4 border-transparent border-b-green-100 animate-spin"
      ...
      <div className="absolute inset-0 overflow-hidden -z-10">
        {[...Array(3)].map((_, i) => (
          <div
            key={i}
            className="absolute rounded-full bg-gradient-to-r from-blue-100/30 to-green-100/30"
            style={{
